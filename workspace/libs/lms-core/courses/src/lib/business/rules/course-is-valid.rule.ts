...
export class CourseIsValidRule extends CompositeRule {
  ...
  configureRules() {
    ...
    if (this.target !== undefined) {
      ...
      this.rules.push(
        new StringIsNotNullEmptyRange('DescriptionIsValid', 'The description value must with within 10 and 600 characters.', this.target.description, 10, 600, this.isDisplayable)
      );
