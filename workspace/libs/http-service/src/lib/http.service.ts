...
export class HttpService {
  ...
  protected handleError(error: HttpErrorResponse): Observable<any> {
    ...
    if (error.error instanceof ErrorEvent) {
      ...
      apiErrorResponse.Errors.push(new ApiErrorMessage(`A client-side or network error occurred. Handle it accordingly.`, true, null));
      ...
      if (error instanceof ErrorApiResponse) {
        ...
        apiErrorResponse.Errors.push(new ApiErrorMessage(`The API returned an unsuccessful response. ${error.status}: ${error.statusText}. ${error.message}`, false, null));
        return throwError(apiErrorResponse);
      }
    }
  }
